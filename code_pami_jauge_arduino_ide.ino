...
const unsigned long delai = 3000; //pour les matchs mettre 85000
...
enum RobotState { ATTENTE, ETAPE_1, ETAPE_2, EVITE, FIN };
...
//Fonction goTo2
bool goTo2(float x_target, float y_target, bool reset = false) {
    ...
    if (phase == 1) {
        ...
        float rot = constrain(err_a * 25.0, -250, 250);
        setMoteurs(rot, -rot);
    }
    else {
        float kp_angle = -230.0;
        float corr = err_a * kp_angle;
       
        if (abs(err_a) > 0.5) { phase = 1; return false; }

        float vit_droit  = 250 - corr - eq;   //toujours à adapter
        float vit_gauche = 250 * COMPENS_GAUCHE + corr;
        setMoteurs(vit_droit, vit_gauche);
    }
    return false;
}
...
void loop() {
    ...
    switch(currentState) {
        ...
        case ETAPE_1:{
            ...
            Serial.println("RETOUR ETAT ETAPE_1");
            ...
            if (!avancement_fixe){
                ...
                float distance = sqrt(pow(600 - pos_x, 2) + pow(0 - pos_y, 2));
                if (distance > 250){
                ...
                if(goTo(619,0)){    //valeur à adapter en fonction de la map, voir pdf
                    avancement_fixe = false;
                    currentState = ETAPE_2;
                }
            }
            break;
        }
        ...
        case ETAPE_2:{
            static bool avancement_fixe2 = false;

            Serial.println("RETOUR ETAT ETAPE_2");

            if (!avancement_fixe2){
                if(goTo2(600,dec3/4)){    //valeur à adapter en fonction de la map, voir pdf
                    goTo2(0,0,true);
                    avancement_fixe2 = true;
                }
            }
            else{
                float distance = sqrt(pow(600 - pos_x, 2) + pow(dec3 - pos_y, 2));  //valeur identique que cella de la ligne 436
                if (distance > 250){
                    if(range2 <= 45){
                        //les 2 lignes ci-dessous permettent de gagner en précision car le robot va s'arrêter et remettre phase=1 (lié au if de la ligne 120)
                        setMoteurs(0,0);
                ...
                if(goTo2(550,dec3)){   //valeur à adapter en fonction de la valeur ligne 436
                    avancement_fixe2 = false;
                    currentState = FIN;
                }
            }
            break;
